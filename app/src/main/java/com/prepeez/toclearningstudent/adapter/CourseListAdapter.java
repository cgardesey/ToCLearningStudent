...
import com.prepeez.toclearningstudent.util.ColorUtils;
...
import static com.prepeez.toclearningstudent.fragment.TabFragment5.myCourses;
...
public class CourseListAdapter extends RecyclerView.Adapter<CourseViewHolder> {
    ...
    public CourseListAdapter(JSONObject jsonObject, String jsonObjKey, int colorIndex, Activity activity) {
    ...
    public CourseViewHolder onCreateViewHolder(final ViewGroup parent, final int viewType) {
        ...
        return new CourseViewHolder(this, LayoutInflater.from(context)
...
//        holder.creditHr.setBackgroundColor(backgroundColorForViewHolder);
//        holder.courseTitle.setBackgroundColor(backgroundColorForViewHolder);
//        holder.add.setBackgroundColor(backgroundColorForViewHolder);
//        holder.arrow.setBackgroundColor(backgroundColorForViewHolder);
        ...
        if (name.equals("---FOOTER---")){
            ...
            holder.optionalCoursesHeader.setVisibility(View.VISIBLE);
            holder.optionalCoursesHeader.setText(context.getResources().getString(R.string.compulsory_course));
            holder.course.setVisibility(View.GONE);
            holder.frameLayout.setVisibility(View.GONE);
        }
        else if (name.equals("OPTIONAL COURSES")){
            holder.optionalCoursesHeader.setVisibility(View.VISIBLE);
            holder.optionalCoursesHeader.setText(context.getResources().getString(R.string.optional_course));
            holder.course.setVisibility(View.GONE);
            holder.frameLayout.setVisibility(View.GONE);
        }
        else {
        ...
        holder.arrow.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(final View v) {
                ...
                if (holder.arrowPointingDown) {
                    ...
                    if (holder.courseListAdapter == null) {
                        try {
                            int index = colorIndex + 1;
                            if (index == 10) {
                                index = 0;
                            }
                            holder.courseListAdapter = new CourseListAdapter(jsonObject.getJSONObject(name), jsonObjKey + "/" + name, index, activity);

                        } catch (JSONException e) {
                            e.printStackTrace();
                        }
                        holder.recyclerView.setAdapter(holder.courseListAdapter);
